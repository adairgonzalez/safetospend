...
export default function Login({ onLogin }) {
  ...
  const handle = async (e) => {
    ...
    try {
      ...
      else { setError(data.error || `Server error (${res.status}) — is the backend running?`); setBusy(false); }
    } catch (err) {
  ...
  return (
    <div className="auth">
      <div className="auth-mark"><WalletIcon width={26} height={26} stroke="#052e16" /></div>
      ...
      <div className="card">
        ...
        <form onSubmit={handle}>
          ...
          <div className="field"><input type="password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} autoComplete="current-password" /></div>
          <button type="submit" className="btn btn-block" disabled={busy}>{busy ? 'Please wait…' : (isRegister ? 'Register' : 'Log in')}</button>
        </form>
        <div className="link-row" style={{marginTop:16}}>
          <button type="button" className="quiet" onClick={()=>{ setIsRegister(!isRegister); setError(null); }}>
